...
def runEpisode(agent, environment, gamma, decision, display, message, pause, episode):
    returns = 0
    totalDiscount = 1.0
    environment.reset()
...
def parseOptions():
    ...
    optParser.add_option('-d', '--gamma',action='store',
                         type='float',dest='gamma',default=0.9,
...
if __name__ == '__main__':
    ...
    a = None
    if opts.agent == 'value':
        a = valueIterationAgents.ValueIterationAgent(mdp, opts.gamma, opts.iters)
    elif opts.agent == 'q':
        #env.getPossibleActions, opts.gamma, opts.learningRate, opts.epsilon
        #simulationFn = lambda agent, state: simulation.GridworldSimulation(agent,state,mdp)
        gridWorldEnv = GridworldEnvironment(mdp)
        actionFn = lambda state: mdp.getPossibleActions(state)
        qLearnOpts = {'gamma': opts.gamma,
        ...
        if not opts.manual and opts.agent == 'value':
            if opts.valueSteps:
                for i in range(opts.iters):
                    tempAgent = valueIterationAgents.ValueIterationAgent(mdp, opts.gamma, i)
    ...
    for episode in range(1, opts.episodes+1):
        returns += runEpisode(a, env, opts.gamma, decisionCallback, displayCallback, messageCallback, pauseCallback, episode)
    if opts.episodes > 0:
        print()
